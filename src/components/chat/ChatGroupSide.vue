  ...
  <div class="chat-group-side">
    ...
    <el-scrollbar class="group-side-scrollbar">
      <div class="group-side-member-list">
        ...
        <div v-for="member in groupMembers" :key="member.id">
          <group-member
            ...
            v-show="!member.quit && member.aliasName.startsWith(searchText)"
            :member="member"
            :showDel="false"
          ></group-member>
        </div>
      ...
      <el-form labelPosition="top" class="group-side-form" :model="group">
        ...
        <div class="btn-group">
          <el-button v-show="editing" type="success" @click="handleSaveGroup()"
            >提交</el-button
          >
          <el-button
            v-show="!editing"
            ...
            >编辑</el-button
          >
          <el-button type="danger" v-show="!isOwner" @click="handleQuit()"
            >退出群聊</el-button
          >
        </div>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    loadGroupMembers() {
      this.$api.getGroupMembers(this.group.id).then((members) => {
    ...
    handleQuit() {
      this.$confirm(
        ...
        this.$api.quitGroup(this.group.id).then(() => {
